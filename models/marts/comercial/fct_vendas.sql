with
    ...
    funcionarios as (select * from {{ ref("dim_funcionarios") }}),
    produtos as (select * from {{ ref("dim_produtos") }}),
    transportadoras as (select * from {{ ref("dim_transportadoras") }}),
    pedido_item as (select * from {{ ref("int_vendas__pedido_itens") }}),
    joined as (
        select
        ...
        left join
            funcionarios on pedido_item.id_funcionario = funcionarios.id_funcionario
        left join produtos on pedido_item.id_produto = produtos.id_produto
        left join
            transportadoras
            on pedido_item.id_transportadora = transportadoras.id_transportadora
    ),
    transformacoes as (
        select
            ...
            *,
            case
                when desconto > 0 then true when desconto = 0 then false else false
